...
ABOUTME: endpoints for real-time observability. Ported from fable branch with
ABOUTME: adversarial-review fixes F1-CR (realpath self-protection), F3-CR (terminal
ABOUTME: file-edit guard), and BR-4/B3 widening applied.
"""
...
# ---------------------------------------------------------------------------
# Guard path resolution helpers (F1-CR fix)
# ---------------------------------------------------------------------------

def _resolve_guard_dirs() -> List[Path]:
    """Return the resolved absolute paths of all protected plugin source dirs.

    Purpose: Determines which directories are write-protected against agent
             modification. Using realpath prevents relative/traversal evasion
             (F1-CR: substring matching is insufficient).
    Usage:   Called lazily at first use; results cached on ControlRoom instance.
    Gotchas: Returns empty list if __file__ resolution fails (defensive).
    """
    plugin_root = Path(__file__).resolve().parent.parent  # plugins/
    guard_dirs: List[Path] = []
    for name in ("control-room", "email-send-guard", "tool-registry-guard"):
        d = (plugin_root / name).resolve()
        guard_dirs.append(d)
    return guard_dirs


def _is_under_guard_dir(path_str: str, guard_dirs: List[Path]) -> bool:
    """Return True if the resolved path falls under any guard directory.

    Purpose: Path normalization check for self-protection (F1-CR fix).
             Uses os.path.realpath to handle relative paths, symlinks, and
             traversal sequences that evade simple substring matching.
    Usage:   Pass raw model-supplied path string; returns True if it targets
             any protected plugin source directory.
    Gotchas: Returns False (allow) if path resolution itself fails — defensive.
    """
    try:
        ...
        for guard_dir in guard_dirs:
            try:
                resolved.relative_to(guard_dir)
                return True
            except ValueError:
                pass
    except Exception:
...
def _resolve_hermes_config() -> Optional[Path]:
    """Return the resolved path to ~/.hermes/config.yaml.

    Purpose: Used to write-protect the hermes config from agent modification
             (B3: agent removing its own key from plugins.enabled).
    Usage:   Called lazily; result may be None if HERMES_HOME resolution fails.
    Gotchas: Uses HERMES_HOME env var if set (matches get_hermes_home behavior).
    """
    try:
        hermes_home_env = os.environ.get("HERMES_HOME")
        if hermes_home_env:
            return (Path(hermes_home_env) / "config.yaml").resolve()
        return (Path.home() / ".hermes" / "config.yaml").resolve()
    except Exception:
        return None


# ---------------------------------------------------------------------------
# Terminal file-edit detection helpers (F3-CR fix)
# ---------------------------------------------------------------------------

# High-signal patterns indicating a terminal command is writing to a file.
# These cover: shell redirects (> and >>), sed -i in-place, tee, cp/mv
# overwrite, printf > ..., python writes. Not exhaustive (per F3-CR rationale:
# full terminal parsing is out of scope for P0-2; residual deferred to P1a).
_TERMINAL_WRITE_PATTERN = re.compile(
    r"""
    (?:
        # Shell redirect operators: > file or >> file
        \s(?:>>?)\s*\S   |
        # sed -i in-place edit
        \bsed\s+(?:-\S+\s+)*-i   |
        # tee command (always writes to its arg)
        \btee\s+   |
        # cp/mv overwriting a destination
        \b(?:cp|mv)\s+(?:-\S+\s+)*\S+\s+\S   |
        # python open(file, 'w') or open(file, 'a') write modes
        \bopen\s*\([^)]*['\"][wa]
    )
    """,
...
def _terminal_targets_protected_path(cmd: str, guard_dirs: List[Path], config_path: Optional[Path]) -> bool:
    """Return True if a terminal command writes to a protected path.

    Purpose: Closes F3-CR gap — terminal-based file edits to guard source
             or config.yaml bypass write_file/patch protection entirely.
    Usage:   Checks high-signal file-write patterns against protected paths.
             Returns False (allow) if command doesn't match write patterns.
    Gotchas: Not exhaustive — coverage targets sed/tee/redirect/cp/mv. Full
             terminal-command parsing deferred to P1a (documented in report).
    """
    if not _TERMINAL_WRITE_PATTERN.search(cmd):
        return False

    # Build set of protected path strings for quick substring matching.
    # Substring matching is acceptable here because we have ALREADY confirmed
    # the command is a write operation (the write-pattern matched above).
    # The risk profile differs from self-protection: we just need to see if
    # the command string mentions a protected path; the command itself is the
    # action, not the path argument to a tool.
    protected_strs: List[str] = []
    for d in guard_dirs:
        protected_strs.append(str(d))
        # Also check the relative plugin dir name (e.g. "plugins/control-room")
        protected_strs.append("plugins/control-room")
        protected_strs.append("plugins/email-send-guard")
        protected_strs.append("plugins/tool-registry-guard")

    if config_path is not None:
        protected_strs.append(str(config_path))
        # Also cover the generic config path patterns
        protected_strs.append(".hermes/config.yaml")
        protected_strs.append("hermes/config.yaml")

    cmd_lower = cmd.lower()
    for ps in protected_strs:
        if ps.lower() in cmd_lower:
            return True
    return False
...
class ControlRoom:
    ...
    def __init__(
        ...
        # Cache guard dirs and config path (computed once, reused per call)
        self._guard_dirs: List[Path] = _resolve_guard_dirs()
        self._config_path: Optional[Path] = _resolve_hermes_config()
    ...
    def _is_config_write(self, path_str: str) -> bool:
        """Return True if the path resolves to ~/.hermes/config.yaml.

        Purpose: Protect hermes config from agent modification (B3).
        Usage:   Called for write_file and patch tool checks.
        Gotchas: Uses realpath to handle traversal (F1-CR parity).
        """
        if not self._config_path:
            return False
        try:
            resolved = Path(os.path.realpath(path_str)).resolve()
            return resolved == self._config_path
        except Exception:
            return False
    ...
    def pre_tool_call(
        ...
        # F1-CR fix: Write-protect security plugin source using realpath comparison,
        # not substring matching. Substring matching on model-supplied args is
        # evadable via relative paths ("__init__.py" from cwd) and traversal sequences.
        # BR-4: Widened to include plugins/tool-registry-guard source.
        # B3: Widened to include ~/.hermes/config.yaml.
        if tool_name in ("write_file", "patch"):
            target_path = args.get("path") or ""
            if _is_under_guard_dir(target_path, self._guard_dirs):
                self.db.log_audit(
                    tool_name, args, "blocked",
                ...
                return {
                    "action": "block",
                    "message": (
                        "Modifying security plugin source code is blocked. "
                        "plugins/control-room/, plugins/email-send-guard/, and "
                        "plugins/tool-registry-guard/ are write-protected."
                    ),
                }
            if self._is_config_write(target_path):
                self.db.log_audit(
                    tool_name, args, "blocked",
                    result="Write to ~/.hermes/config.yaml blocked",
                    task_id=task_id, session_id=session_id,
                )
                return {
                    "action": "block",
                    "message": (
                        "Modifying ~/.hermes/config.yaml is blocked. "
                        "The hermes configuration file is write-protected to prevent "
                        "disabling governance plugins at runtime."
                    ),
                }

        # F3-CR fix: Block terminal commands that write to guard source or config.yaml.
        # write_file/patch protection does not cover terminal redirects, sed -i, tee, etc.
        # This adds high-signal coverage for the most dangerous forms.
        # Residual gap: full terminal command parsing is out of scope for P0-2;
        # obfuscated forms (bash -c 'eval ...') are deferred to P1a egress/credential wall.
        if tool_name == "terminal":
            cmd = args.get("command") or ""
            if _terminal_targets_protected_path(cmd, self._guard_dirs, self._config_path):
                self.db.log_audit(
                    tool_name, args, "blocked",
                    result="Terminal write to protected guard/config path blocked",
                    task_id=task_id, session_id=session_id,
                )
                return {
                    "action": "block",
                    "message": (
                        "Terminal commands that modify security plugin source or hermes config are blocked. "
                        "Affected paths: plugins/control-room/, plugins/email-send-guard/, "
                        "plugins/tool-registry-guard/, ~/.hermes/config.yaml. "
                        "Use the official update mechanisms instead."
                    ),
                }
        ...
        # F2-CR note: This is substring-based steering, not hard security.
        # __import__("agent"), importlib.import_module("gateway") evade it.
        # Real containment depends on the execute_code sandbox (P1a). Documented
        # as steering-only; B10 is NOT claimed as a hard security close.
