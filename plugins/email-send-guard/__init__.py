...
def _handle_approve(
    ...
    **_kw: Any,
    ...
    """
    logger.debug("[EMAIL-TRACE] _handle_approve called with raw_args=%s", raw_args)
    ...
    logger.debug("[EMAIL-TRACE] _handle_approve: draft_id=%s, recipient=%s", draft_id, recipient)
    if not recipient:
    ...
    logger.debug("[EMAIL-TRACE] user_message=%s", user_message)
    logger.debug("[EMAIL-TRACE] followup_agent_message=%s", followup)
    result = {
    ...
    logger.debug("[EMAIL-TRACE] _handle_approve returning dict: %s", result)
    return result
